...
// _userProfile is accepted for interface stability but not yet consumed by the model.
export function buildStudentModel(learningMd, curriculum, _userProfile) {
...
function parseEngagement(learningMd) {
  ...
  // The lesson log records minimal | brief | engaged | high; both disengaged labels are this model's "low"
  const logged = learningMd.match(/\*\*Engagement:\*\*\s*(\w+)/i)?.[1].toLowerCase();
  if (logged === 'minimal' || logged === 'brief') return 'low';
  if (logged === 'engaged' || logged === 'high' || logged === 'low') return logged;
...
// Lesson grades that make a lesson's concepts count as shaky.
const SHAKY_GRADES = ['incorrect', 'minimal'];

/**
 * A completed review is the retest a shaky concept was waiting for. Re-grade the
 * lessons that flagged it, otherwise the practitioner re-issues the same BLOCK
 * after every review and the student can never advance.
 */
export function markConceptReviewed(curriculum, concept) {
  for (const lesson of curriculum?.lessons || []) {
    if ((lesson.concepts || []).includes(concept) && SHAKY_GRADES.includes(lesson.engagement)) {
      lesson.engagement = 'reviewed';
    }
  }
  return curriculum;
}
...
function classifyConcepts(exercises, curriculum) {
  ...
  // Track the LATEST status per concept (last mention wins)
  const conceptStatus = new Map();
  for (const lesson of completed) {
    const concepts = lesson.concepts || [];
    const engagement = lesson.engagement;
    for (const c of concepts) {
      if (engagement === 'correct' || engagement === 'delivered' || engagement === 'high' || engagement === 'engaged') {
        conceptStatus.set(c, 'solid');
      } else if (SHAKY_GRADES.includes(engagement)) {
        conceptStatus.set(c, 'shaky');
      }
    }
  }
  ...
  for (const [concept, status] of conceptStatus) {
    if (status === 'solid') solid.push(concept);
    else shaky.push(concept);
  }

  return {
    solid: solid.slice(0, 10),
