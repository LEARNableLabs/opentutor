...
export class SupabaseStore {
  ...
  // Keys under `prefix` that sort before `prefix + bound`: a bounded delete that needs no
  // listing first. assertKvPrefix refuses a prefix that could reach every row.
  async deleteKVBefore(prefix, bound) {
    const pattern = assertKvPrefix(prefix) + '%'; // before any query is built
    const { error } = await this.supabase.from('kv').delete().eq('user_id', this._uid)
      .like('key', pattern).lt('key', prefix + String(bound));
    if (error) throw error;
  }
  ...
  // ── Progress ──────────────────────────────────────────────
  // Profile and progress go through readKV/writeKV, which throw on a database
  // error (#157). `.single()` with the error ignored made a failed read look like
  // "no progress yet", and updateProgress then wrote the default over the real one.

  async readProgress() {
    return (await this.readKV('progress')) ?? { active_topics: [], schedule: {}, history: [], onboarding: null };
  }

  async writeProgress(progressData) {
    await this.writeKV('progress', progressData);
  }
  ...
  async readUser() {
    return (await this.readKV('user_profile')) ?? '';
  }

  async writeUser(content) {
    await this.writeKV('user_profile', content);
  }
