...
export class SupabaseStore {
  ...
  async readKV(key) {
    const { data } = await this.supabase.from('kv').select('value').eq('key', key).maybeSingle();
    return data?.value ?? null;
  }

  async writeKV(key, value) {
    await this.supabase.from('kv').upsert({ key, value });
  }

  async deleteKV(key) {
    await this.supabase.from('kv').delete().eq('key', key);
  }
  ...
  // ponytail: completion still goes to the filesystem, which is read-only on
  // serverless — moving it to the `lessons_completed` table is part of #94.
  readCurriculum(topicSlug) {
    return readCurriculumWithProgress(this.paths.domains, path.join(this.root, 'workspace'), topicSlug);
  }
  ...
  async markLessonComplete(topicSlug, day, engagement = 'delivered') {
    recordCompletion(completionsFile(path.join(this.root, 'workspace')), topicSlug, day, engagement);
  ...
  readDomainFile(topicSlug, filename) {
    try { return fs.readFileSync(domainFilePath(this.paths.domains, path.join(this.root, 'workspace'), topicSlug, filename), 'utf-8'); }
    catch { return null; }
  }

  writeDomainFile(topicSlug, filename, content) {
    const p = domainFilePath(this.paths.domains, path.join(this.root, 'workspace'), topicSlug, filename);
