...
import { completionsFile, recordCompletion, readCurriculumWithProgress, withoutRuntimeFields, domainFilePath, workspaceDir } from './progress.js';
...
export class SupabaseStore {
  ...
  constructor(rootDir, options = {}) {
    ...
    // equals NULL, so a null-scoped row could never be read back.
  ...
  async readKV(key) {
    const { data } = await this.supabase.from('kv').select('value').eq('user_id', this._uid).eq('key', key).maybeSingle();
    return data?.value ?? null;
  }

  async writeKV(key, value) {
    await this.supabase.from('kv').upsert({ user_id: this._uid, key, value });
  }

  async deleteKV(key) {
    await this.supabase.from('kv').delete().eq('user_id', this._uid).eq('key', key);
  }
