...
import { completionsFile, readCompletions, applyCompletions, recordCompletion, readCurriculumWithProgress, withoutRuntimeFields, domainFilePath, seedFromTemplate, workspaceDir, assertUserId, assertKvPrefix } from './progress.js';
...
export class TutorStore {
  ...
  deleteKVBefore(prefix, bound) { this.db.prepare('DELETE FROM kv WHERE user_id=? AND key LIKE ? AND key < ?').run(this._uid,assertKvPrefix(prefix)+'%',prefix+String(bound)); }
