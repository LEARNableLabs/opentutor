...
import { completionsFile, recordCompletion, readCurriculumWithProgress, withoutRuntimeFields, domainFilePath, seedFromTemplate } from './progress.js';
...
export class TutorStore {
  constructor(rootDir) {
    ...
    this._stmts = {
      ...
      delKV: this.db.prepare('DELETE FROM kv WHERE key = ?'),
  ...
  readKV(key) {
    return this._stmts.getKV.get(key)?.value ?? null;
  }

  writeKV(key, value) {
    this._stmts.setKV.run(key, value);
  }

  deleteKV(key) {
    this._stmts.delKV.run(key);
  }
