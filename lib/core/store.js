...
export class TutorStore {
  ...
  // A LIKE pattern, as in listKV: keep `_` and `%` out of the prefix.
  deleteKVPrefix(prefix) { this.db.prepare('DELETE FROM kv WHERE user_id=? AND key LIKE ?').run(this._uid,prefix+'%'); }
