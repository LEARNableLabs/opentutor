...
export async function prepareTopicBuild(state, payload) {
...
export async function runTopicBuildStep({ state, adapter, getAdapter, skills, slug, id, seq, quickStart = generateQuickStart, now = Date.now }) {
  ...
  try {
    ...
    const failed = nextVersion(doc, { lease: null, status: doc.attempts >= 3 ? 'failed' : 'queued', error: 'The curriculum build hit a problem and will retry.' });
    await state.compareAndSetTopic(slug, doc, failed);
    throw err;
  }
}
