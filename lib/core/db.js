...
function migrate(db) {
  ...
  // A leftover kv_v1 means a previous run was interrupted mid-rebuild. Recover
  // before anything else: without this, a crash after the RENAME left no `kv`
  // at all, the check below took the fresh-database path, and CREATE TABLE made
  // an empty one while the profile and progress sat stranded in kv_v1.
  if (has('kv_v1')) {
    db.transaction(() => {
      db.exec(`
        CREATE TABLE IF NOT EXISTS kv (
        ...
        INSERT OR IGNORE INTO kv (user_id, key, value) SELECT '', key, value FROM kv_v1;
        DROP TABLE kv_v1;
      `);
    })();
  }

  if (!has('kv')) return;   // fresh database; SCHEMA creates it correctly
  ...
  // One transaction, so an interrupted rebuild rolls back instead of stranding
  // the data — and so a second process starting at the same moment sees either
  // the old shape or the new one, never the half-renamed middle.
  db.transaction(() => {
    db.exec(`
