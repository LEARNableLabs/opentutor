...
export async function addTopic({ state, adapter, getAdapter, skills, enqueue, quickStart }, payload) {
  // Ask for the model before a new build is recorded, so a refusal leaves nothing behind (#132).
  const { existing, doc } = await prepareTopicBuild(state, payload, { beforeBuild: async () => { adapter ||= await getAdapter?.(); } });
