...
import fs from 'fs';
import { accountKey, decodeAccount, listAccounts } from './accounts.js';
import { revokeStudentToken } from './student-auth.js';
...
export async function listStudents(store) {
  return [...await read(store), ...await listAccounts(store)];
}

/** One student, or null. */
export async function findStudent(store, userId) {
  const id = assertUserId(userId);
  if(id.startsWith('acct-')) return decodeAccount(await store.readKV(accountKey(id)));
...
export async function provisionStudent(store, userId, meta = {}) {
  const id = assertUserId(userId);
  if(id.startsWith('acct-')) throw new Error('Invalid student id: acct- is reserved for accounts');
...
export async function decommissionStudent(store, userId) {
  const id = assertUserId(userId);
  const account = id.startsWith('acct-') ? decodeAccount(await store.readKV(accountKey(id))) : null;
  const students = await read(store);

  const remaining = students.filter((s) => s.id !== id);
  if (!account && remaining.length === students.length) {
    throw new Error(`Student ${id} not found`);
  }

  // Revoke first. A failed cleanup must never leave the old credential usable.
  if(account)await store.writeKV(accountKey(id),JSON.stringify({...account,status:'disabled'}));
  await revokeStudentToken(store, id);
  const theirs = store.forStudent(id);
  try {
    if (theirs.deleteAllStudentState) await theirs.deleteAllStudentState();
  } finally {
    theirs.close?.();
  }
  // File backends keep completions, logs and file KV in this directory.
  if (!store.supabase) {
    const dir = workspaceDir(store.dataDir || store.root, id);
    fs.rmSync(dir, { recursive: true, force: true });
  }
  if(!account)await write(store, remaining);
