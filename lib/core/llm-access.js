...
// Each unit of the trial is its own row, and only one request can create a given row
// (insert-if-absent, then read back who won), so the caps hold across concurrent
// requests and serverless instances without a lock.
async function claimSlot(state, kind, limit) {
  const prefix = `${TRIAL}${kind}:`;
...
function metered(host, state, kind, limit, reason) {
  ...
  adapter.generate = async (...args) => {
    if (!slot) {
      slot = await claimSlot(state, kind, limit);
...
/** One conversational turn from the browser, bounded like the history (null if it isn't text). */
export const turnText = (text) => (typeof text === 'string' ? text.slice(0, 4000) : null);
