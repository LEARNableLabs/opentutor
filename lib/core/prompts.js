 ...
 */
export function buildSocraticResponsePrompt(lessonPlan, studentAnswer, step, user, { final = false } = {}) {
...
You are mid-conversation with a student. Respond to their answer naturally — warm, concise, and specific to what they said. Never generic.`,
    untrustedData('lesson-plan', JSON.stringify(lessonPlan), 8_000),
    untrustedData('student-profile', user || '', 2_000),
    `## Current Step: ${step}

${final ? closing : stepInstructions[step] || stepInstructions.diagnostic}
 ...
 */
export function buildOnboardingPrompt(skills, userProfile = '', { availableTopics } = {}) {
  const reference = skills?.get?.('onboarding') || '';
  const catalogOnly = Array.isArray(availableTopics);
  ...
  const system = [
    ...
    catalogOnly ? [
      ...
      'Suggest only topics from the available list below, using readable names in conversation.',
      ...
      'If the list is empty, say that no curricula are currently available and do not confirm a topic.',
