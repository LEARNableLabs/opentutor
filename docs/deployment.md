...
The example lesson on `/` lets a visitor who is not signed in answer its question and get one reply from the tutor, on the deployment's key and cheap model, before being offered signup (#153). `POST /api/demo` is served by the catalog function through a `vercel.json` rewrite, and by the standalone server directly. It accepts same-origin requests only, answers of 1–300 characters, and caps each reply at 500 characters. Replies are limited to **3 per IP address and 300 a day in total** (UTC days; `DEMO_PER_IP` and `DEMO_PER_DAY` in `api/_lib/demo.js`). Each reply claims its slots before the model is called, so the caps hold under concurrent requests; past either cap the route answers 429 and the page offers signup instead. A browser that has had its reply shows the signup offer instead of the form.

The IP address is never stored: only a truncated HMAC of it, keyed with `SUPABASE_SECRET_KEY` / `SUPABASE_SERVICE_ROLE_KEY` (else `OPENTUTOR_PASSWORD`) and the day, in `demo:` rows of the unnamed store. Each day's first reply deletes earlier days' rows. The address is the first `X-Forwarded-For` entry, which Vercel sets itself. Behind a proxy that appends to that header instead, or on a standalone server reached directly, a client can choose its own address, and only the daily cap binds.
